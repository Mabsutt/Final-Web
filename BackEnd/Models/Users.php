    ...
    public function Loggin($email,$condition){
      ...
      //  echo $row_count;
        ...
        if ($row_count > 0) {
            ...
            $_SESSION['USERNAME'] = $userData['DATE_OF_BIRTH'];
            $_SESSION['PHONE_NUMBER'] = $userData["PHONE_NUMB"];
...
public function PostBook($userID, $bookName, $bookDescription, $imagePath) {
    // Prepare the SQL statement using placeholders to avoid SQL injection
   

    $query = "INSERT INTO Books (BOOK, DESCRIPTION, IS_ACTIVE, USERID, IMAGE_PATH) VALUES ('$bookName', '$bookDescription', 1,'$userID' , '$imagePath')";
    ...
    case 'POSTBOOK':
        ...
        if ($bookImage && $bookImage['error'] === UPLOAD_ERR_OK) {
            ...
            if(move_uploaded_file($fileTmpPath, $dest_path)) {
                $imagePath = $dest_path; // Adjust if you need a different path for frontend access
            } else {
        ...
        if ($userID && $bookName && $bookDescription && isset($imagePath)) {
            ...
            $controller->PostBook($userID, $bookName, $bookDescription, $imagePath);
        } else {
